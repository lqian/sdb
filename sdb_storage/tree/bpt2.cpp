...
void _inode::set_left_ipage(_ipage * p) {
	...
	sdb::tree::set_flag(header->flag, INODE_LEFT_IPAGE_BIT);
...
void _inode::set_right_ipage(_ipage *p) {
	...
	sdb::tree::set_flag(header->flag, INODE_RIGHT_IPAGE_BIT);
...
void _inode::set_left_lpage(_lpage *p) {
	...
	header->left_pg_off = p->offset;
	header->left_pg_seg_id = p->seg->get_id();
	sdb::tree::set_flag(header->flag, INODE_LEFT_LPAGE_BIT);
...
void _inode::set_right_lpage(_lpage *p) {
	...
	header->right_pg_off = p->offset;
	header->right_pg_seg_id = p->seg->get_id();
	sdb::tree::set_flag(header->flag, INODE_RIGHT_LPAGE_BIT);
...
void _ipage::set_parent(const _inode * n) {
	...
	sdb::tree::set_flag(header->flag, PAGE_PARENT_BIT);
}
...
int fsn_ipage::remove_inode(ushort idx) {
	...
	if (off <= length) {
		...
		sdb::tree::set_flag(n.header->flag, INODE_REMOVE_BIT);
	} else {
		r = sdb::FAILURE;
	}
	return r;
...
int fsn_lpage::remove_lnode(ushort idx) {
	...
	if (off <= length) {
		fs_lnode n;
		n.ref(buffer + off, ns);
		sdb::tree::set_flag(n.header->flag, LNODE_REMOVE_BIT);
	} else {
		r = sdb::FAILURE;
	}
	return r;
